...
export default async function PlexShowPage({
  ...
  const { media, knownIds, tmdbMedia, posters, logos } = await buildPosters(id, "show");

  if (!tmdbMedia) {
    return (
      <div className="flex flex-col items-center justify-cente mw-[600px] mx-auto">
      <div className="text-2xl font-bold">Damn, we could not find this one..</div>
        <div className="mt-4 text-sm">
          <p>TMDB ID: {knownIds.tmdbId || 'N/A'}</p>
          <p>IMDB ID: {knownIds.imdbId || 'N/A'}</p>
          <p>TVDB ID: {knownIds.tvdbId || 'N/A'}</p>
          <p className="mt-4 border-t border-gray-200 pt-4 text-sm font-bold">This just means we can&apos;t connect the data listed above to TMDB.<br />You should probably have Plex &quot;Match&quot; this show again.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <MediaHeader media={media} logos={logos} mediaType="show" />
      <hr className="my-4" />
      <h2 className="my-4 text-2xl font-bold">Posters</h2>
      <PosterPicker posters={posters} ratingKey={id} mediaType="show" />
    </div>
  );
}
